...
esp_err_t set_esp_interface_dns(esp_interface_t interface,
                                IPAddress main_dns = IPAddress(),
                                IPAddress backup_dns = IPAddress(),
                                IPAddress fallback_dns = IPAddress());
esp_err_t set_esp_interface_ip(esp_interface_t interface,
...
static void
wifi_sta_config(wifi_config_t *wifi_config, const char *ssid = NULL,
                const char *password = NULL, const uint8_t *bssid = NULL,
                uint8_t channel = 0,
                wifi_scan_method_t scan_method = WIFI_ALL_CHANNEL_SCAN,
                wifi_sort_method_t sort_method = WIFI_CONNECT_AP_BY_SIGNAL,
                uint16_t listen_interval = 0, bool pmf_required = false) {
  wifi_config->sta.channel = channel;
  wifi_config->sta.listen_interval = listen_interval;
  wifi_config->sta.scan_method =
      scan_method; // WIFI_ALL_CHANNEL_SCAN or WIFI_FAST_SCAN
 ...
 * if passphrase is set the most secure supported mode will be automatically
 * selected
 * @param ssid const char*          Pointer to the SSID string.
 * @param passphrase const char *   Optional. Passphrase. Valid characters in a
 * passphrase must be between ASCII 32-126 (decimal).
 * @param bssid uint8_t[6]          Optional. BSSID / MAC of AP
 * @param channel                   Optional. Channel of AP
 * @param connect                   Optional. call connect
 * @return
 */
wl_status_t WiFiSTAClass::begin(const char *ssid, const char *passphrase,
  ...
  conf.sta.scan_method =
      WIFI_ALL_CHANNEL_SCAN; // force full scan to be able to choose the nearest
                             // / strongest AP
  ...
  if (esp_wifi_get_config((wifi_interface_t)ESP_IF_WIFI_STA, &current_conf) !=
      ESP_OK) {
 ...
 */
bool WiFiSTAClass::config(IPAddress local_ip, IPAddress gateway,
 ...
 */
bool WiFiSTAClass::isConnected() { return (status() == WL_CONNECTED); }
...
bool WiFiSTAClass::getAutoReconnect() { return _autoReconnect; }
...
IPAddress WiFiSTAClass::broadcastIP() {
  ...
  return WiFiGenericClass::calculateBroadcast(IPAddress(ip.gw.addr),
                                              IPAddress(ip.netmask.addr));
}
...
IPAddress WiFiSTAClass::networkID() {
  ...
  return WiFiGenericClass::calculateNetworkID(IPAddress(ip.gw.addr),
                                              IPAddress(ip.netmask.addr));
}
...
String WiFiSTAClass::BSSIDstr(void) {
  ...
  char mac[18] = {0};
  sprintf(mac, "%02X:%02X:%02X:%02X:%02X:%02X", bssid[0], bssid[1], bssid[2],
...
bool WiFiSTAClass::enableIpV6() {
  ...
  return esp_netif_create_ip6_linklocal(
             get_esp_interface_netif(ESP_IF_WIFI_STA)) == ESP_OK;
}
...
IPv6Address WiFiSTAClass::localIPv6() {
  ...
  if (esp_netif_get_ip6_linklocal(get_esp_interface_netif(ESP_IF_WIFI_STA),
                                  &addr)) {
    return IPv6Address();
  }
...
bool WiFiSTAClass::beginSmartConfigV2() {
  ...
  _smartConfigType = SC_TYPE_ESPTOUCH_V2;
  err = esp_smartconfig_start(&conf);
  if (err != ESP_OK) {
    log_e("SmartConfig Start Failed!");
    return false;
